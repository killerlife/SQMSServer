...
SMTPSETTING SQMSDatabase::GetSmtpSetting()
{
	...
	if (db.isOpen())
	{
		...
		{
			mSS.strSender = model.data(model.index(0, 0)).toString();
			mSS.strPwd = model.data(model.index(0, 1)).toString();
			mSS.strServer = model.data(model.index(0, 2)).toString();
			mSS.nPort = model.data(model.index(0,3)).toInt();
			mSS.strRecvList = model.data(model.index(0,4)).toString();
			mSS.mStartTime = model.data(model.index(0, 5)).toTime();
		}
		CloseDB();
	}
	nFlag = 0;
...
void SQMSDatabase::GetQDevList(TQDevItemList& itemList)
{
	...
	if (db.isOpen())
	{
		...
		{
			item.strTheaterNo = model.data(model.index(i, 0)).toString();
			item.strIp = model.data(model.index(i, 1)).toString();
			if (model.data(model.index(i, 14)).toInt() == 0)
				item.nPort = 10001;
			else
				item.nPort = model.data(model.index(i, 14)).toInt();

			item.mDevType = DEV_LLAS100;
			item.fix_ftl = model.data(model.index(i, 2)).toDouble();
			item.fix_lc = model.data(model.index(i, 3)).toDouble();
			item.fix_rc = model.data(model.index(i, 4)).toDouble();
			item.fix_cc = model.data(model.index(i, 5)).toDouble();
			item.fix_hb = model.data(model.index(i, 6)).toDouble();
			item.fix_la = model.data(model.index(i, 7)).toDouble();
			item.fix_ra = model.data(model.index(i, 8)).toDouble();
			item.tBefore = model.data(model.index(i, 9)).toTime();
			item.tAfter = model.data(model.index(i, 10)).toTime();
			item.strTheaterName = model.data(model.index(i, 11)).toString();
			//item.mSyncMode = MODE_SIS;
			item.mSyncMode = (MODESYNC)model.data(model.index(i, 13)).toInt();
			item.strScript = model.data(model.index(i, 12)).toString();
...
void SQMSDatabase::SaveCollectData(TValueItemList *pList, QMSDEVICE& mDev)
{
	...
	if (db.isOpen())
	{
		...
		if (model.lastError().isValid())
		{
			...
			pLog->Write(LOG_DATABASE, model.lastError().text());
			CloseDB();
			nFlag = 0;
			return;
		}
